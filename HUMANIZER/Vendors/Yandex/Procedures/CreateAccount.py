...
class CreateAccount:
    ...
    def __enterData(self):
        ...
        enterFirstName = self.__browser.clickAndInput(self.__browser.findByCss('input[name="firstname"]'), value=self.__account.FirstName, interval=1)
        enterLastname = self.__browser.clickAndInput(self.__browser.findByCss('input[name="lastname"]'), value=self.__account.LastName, interval=2)
        clickOnLogin = self.__browser.clickAndInput(self.__browser.findByCss('input[name="login"]'), value=self.__account.LastName, interval=2)
        enterPassword = self.__browser.clickAndInput(self.__browser.findByCss('input[name="password"]'), value=self.__account.Password, interval=2)
        enterPasswordConfirmation = self.__browser.clickAndInput(self.__browser.findByCss('input[name="password_confirm"]'), value=self.__account.Password, interval=2)
        enterSecurityQuestion = self.__browser.clickAndInput(self.__browser.findByCss('input[name="hint_answer"]'), value=self.__account.SecurityHint, interval=2)

        captchaCode = input('Enter captcha > ')
        enterCaptcha = self.__browser.clickAndInput(self.__browser.findByCss('input[name="captcha"]'), value=captchaCode, interval=2)
